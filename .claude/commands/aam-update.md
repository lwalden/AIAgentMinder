...
6. **`Stop` hook — sprint continuation enforcement:** Ensure the `hooks.Stop` array contains an entry with `"command": "bash .claude/scripts/sprint-stop-guard.sh"`. Same merge logic: add if missing, update if present, preserve user-added entries.

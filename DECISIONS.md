...
### Recreate the pre-PR quality gate hook (`pre-pr-gate-hook.sh`) | 2026-06-04 | Status: Active

Chose: Re-add a `PreToolUse` / `matcher: "Bash"` hook (`bin/pre-pr-gate-hook.sh`) that blocks `gh pr create` / `mcp__github__create_pull_request` unless (a) `.quality-gate-pass` exists and is fresh (default 60 min, `AAM_PR_GATE_TTL_SECONDS`), (b) `.quality-review-result.json`, if present, does not say `decision: "block"`, and (c) the PR diff's added lines contain no high-confidence hardcoded secret. The v4.3 soft commit-gate is **not** revived (its `.tests-passed` marker has no writer in v5.x). Fail-open on missing `jq`/`git`/errors; per-session opt-out via `AAM_PR_GATE_BYPASS=1`.

The secret scan (gate c) is the resolution of the "migrate deterministic checks to hooks" question, deliberately scoped narrow: it runs at PR-creation time (not per-write), scans **only added diff lines**, and matches **only distinctive credential formats** (AWS `AKIA…`, GitHub `ghp_`/`github_pat_`, Google `AIza…`, Slack `xox*-`, Stripe `sk_live_`, PEM private-key headers). No generic `password=`/`api_key=`/entropy heuristics — those are the security-reviewer lens's job and would make a hard gate too noisy. Base ref is resolved by trying `origin/HEAD`→`origin/main`→`origin/master`→`main`→`master`; if none resolve, the scan is skipped (fail open). Disable just this gate with `AAM_PR_GATE_SECRETS=0`.

Why: The write side already existed and was orphaned — `/aiagentminder:quality-gate` writes `.quality-gate-pass`, `quality-reviewer` emits a block/pass verdict, and both markers were already in `templates/.gitignore` — but **nothing read them**, so the README's "deterministic enforcement of quality" was prose-only at the PR boundary. A working version of this exact hook shipped in v4.3.0 (PR #143) and was deleted in v5.0 prep (commit `d4415fa`, 2026-04-12) under the label "empty placeholder with no implementation." That label is **inaccurate**: the git diff of the deletion shows a full 75-line implementation (hard PR gate + soft commit gate), and the DECISIONS entry one day earlier (2026-04-11, settings-merge) treated it as a live, shipping feature. The hook was swept out alongside the genuinely-retired `correction-capture`/`context-cycle` hooks during the `project/.claude/scripts/` + `settings.json.tpl` teardown; there is no decision on record that the approach was wrong. The agent/skill text that kept referencing "the PreToolUse PR gate hook" (`quality-reviewer.md`, `quality-gate`) survived precisely because it described a real, intended mechanism.

While wiring this up, fixed a latent contradiction: `quality-reviewer` is read-only (`disallowedTools: [Edit, Write, Bash]`) yet was instructed to write `.quality-review-result.json` via a Bash `echo` it cannot run — so Gate 2 never had a file to read. Resolved by having the reviewer emit its result line in its output and the caller (sprint-master TEST state / `self-review` skill) persist it with the Write tool.

Alternatives considered: (a) a `PostToolUse(Write|Edit)` per-write scanner for secrets/debug-statements/file-size — rejected; runs on every write (the cost/Windows profile that made the retired `context-cycle-hook` painful), is stack-specific, and is noisy. The PR-time secret scan above captures the highest-value, lowest-false-positive subset without that cost. (b) Delete the orphaned markers and "hook reads this" claims instead of rebuilding — rejected; it forfeits mechanical enforcement, which is a core product claim. (c) Gate only the Bash `gh pr create` path and leave MCP-tool PR creation uncovered — rejected after self-review surfaced that the header/CHANGELOG claimed MCP coverage the `Bash`-only matcher couldn't deliver. The hook is now registered under two matchers (`Bash` and `mcp__github__create_pull_request`) and the trigger handles both: a Bash command matching `gh pr create`, or the MCP tool matched by name (its call carries no command string to grep). If a user's GitHub MCP server is named differently, the matcher simply doesn't fire (fail-open, no worse than before).

Gate 2 is best-effort: only flows that run the quality-reviewer judge (sprint TEST state, `/self-review`) write `.quality-review-result.json`; the pr-pipeliner baseline review does not, and relies on Gate 1. The "mechanically enforce the review decision" claim is therefore scoped to the judge flows.

Tradeoff: the PreToolUse hook runs on every Bash call (and on the MCP tool), but it fast-path exits unless the call is a PR creation (same shape as `sprint-phase-guard.sh`). Manual users who run `gh pr create` without first running the quality gate are now blocked — intended, with `AAM_PR_GATE_BYPASS=1` and the quality-gate override path (which now records the marker) as the two escape hatches. Hooks don't run on Claude Code web, so enforcement is CLI/IDE only (consistent with the rest of the plugin). 17 unit tests; full suite green.

...
Chose: Re-add a `PreToolUse` / `matcher: "Bash"` hook (`bin/pre-pr-gate-hook.sh`) that blocks `gh pr create` / `mcp__github__create_pull_request` unless (a) `.quality-gate-pass` exists and is fresh (default 60 min, `AAM_PR_GATE_TTL_SECONDS`) and (b) `.quality-review-result.json`, if present, does not say `decision: "block"`. Single-purpose hard gate only; the v4.3 soft commit-gate is **not** revived (its `.tests-passed` marker has no writer in v5.x). Fail-open on missing `jq`/errors; per-session opt-out via `AAM_PR_GATE_BYPASS=1`.

...
| `pre-pr-gate-hook.sh` | PreToolUse (`Bash` + `mcp__github__create_pull_request`) | Blocks PR creation (`gh pr create` or the GitHub MCP tool) until the quality gate passed, the review judge didn't block, and the diff adds no high-confidence secret; fail-open, `AAM_PR_GATE_BYPASS=1` to skip |

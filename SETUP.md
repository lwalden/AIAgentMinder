...
### AGENTS.md -- Project Identity Section
...
- DECISIONS.md (architectural decisions -- shared across all agents)
- docs/strategy-roadmap.md (planning template)
- docs/agent-setup.md (per-agent setup reference)
- .claude/settings.json (Claude Code permissions + hooks)
- .claude/commands/ (2 commands: /plan, /checkpoint -- Claude Code only)
- .claude/hooks/ (4 Node.js hooks: timestamp, auto-commit, context re-injection, auto-lint)
- .gitignore (core + [stack] entries)

Next steps:
1. Open your target project with your preferred AI agent
2. Claude Code: Run /plan to create your strategy roadmap
   Other agents: Say "Read PLAN.md and follow the instructions"
3. Or say "Read AGENTS.md and PROGRESS.md, then resume" if you already have work in progress
```

...
/**
 * Detect an existing AIAgentMinder installation in the target directory.
 *
 * @param {string} dir - Project root directory
 * @returns {{ installed: boolean, version: string|null }}
 */
export function detectExistingInstall(dir) {
  // Check for version stamp (strongest signal)
  const versionContent = readIfExists(dir, '.claude', 'aiagentminder-version');
  if (versionContent) {
    return { installed: true, version: versionContent.trim() };
  }

  // Check for aam- prefixed skills
  const skillsDir = path.join(dir, '.claude', 'skills');
  if (fs.existsSync(skillsDir)) {
    const entries = fs.readdirSync(skillsDir);
    if (entries.some(e => e.startsWith('aam-'))) {
      return { installed: true, version: null };
    }
  }

  // Check for AIAgentMinder-managed rules
  const rulesDir = path.join(dir, '.claude', 'rules');
  if (fs.existsSync(rulesDir)) {
    const entries = fs.readdirSync(rulesDir).filter(e => e.endsWith('.md'));
    for (const entry of entries) {
      const content = readIfExists(dir, '.claude', 'rules', entry);
      if (content && content.includes('AIAgentMinder-managed')) {
        return { installed: true, version: null };
      }
    }
  }

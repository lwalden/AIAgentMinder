...
/**
 * Validate that all skill directories contain a SKILL.md file.
 *
 * @param {string} repoDir
 * @returns {{ valid: boolean, errors: string[], skillCount: number, skills: string[] }}
 */
export function validateSkillPackages(repoDir) {
  const errors = [];
  const skillsDir = path.join(repoDir, 'skills');

  if (!fs.existsSync(skillsDir)) {
    return { valid: false, errors: ['skills/ directory not found'], skillCount: 0, skills: [] };
  }
  ...
  for (const dir of entries) {
    ...
    if (!fs.existsSync(skillMdPath)) {
      errors.push(`skills/${dir}/ is missing SKILL.md`);
    }
  }
...
export function validateAll(repoDir) {
  return {
    ...
    skills: validateSkillPackages(repoDir),

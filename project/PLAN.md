...
> **For non-Claude agents:** Read this file and follow the instructions to create your project strategy roadmap.
> **Claude Code users:** Run `/plan` instead — this file is not needed.

You are helping the user create or update `docs/strategy-roadmap.md` for this project.
This document is the "north star" for development -- it tells the agent the "why" behind decisions.

---

## Before Starting

Read `AGENTS.md` to understand the project identity (name, type, stack).
If Project Identity is still placeholder brackets, run setup first (Claude Code: `/setup`; other agents: `"Read SETUP.md and follow the instructions"` in the template repo).
...
3. Populate `## MVP Goals` in `AGENTS.md` with Phase 1 deliverables (3-5 testable bullet points)
4. Apply the same MVP Goals to the `## MVP Goals` section in `.github/copilot-instructions.md`
5. Update `PROGRESS.md` to note that strategy roadmap was created
6. If MCP servers were mentioned, add `**MCP Servers:**` line to Project Identity in `AGENTS.md` and `.github/copilot-instructions.md`
7. Tell the user: "Your roadmap is ready. Tell me to start Phase 1 when you're ready."

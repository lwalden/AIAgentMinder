# Product Brief & Roadmap

> The "why" and "what" behind this project. Claude references this on-demand for decision context.
> Run `/aiagentminder:brief` to fill this in interactively, or edit manually.

---

## What & Why

**Problem:** [What problem does this solve? Who has it?]
**Vision:** [One sentence — what does success look like?]
**Target Users:** [Who will use this?]

---

## MVP Features (Phase 1)

1. [Feature] -- Acceptance: [testable criterion]
2. [Feature] -- Acceptance: [testable criterion]
3. [Feature] -- Acceptance: [testable criterion]

**Out of Scope:** [What this project is NOT doing — be specific. "Won't support X" not "won't do everything."]

---

## Technical Stack

| Component | Choice | Why |
|-----------|--------|-----|
| [e.g. Backend] | [e.g. Node/Express] | [rationale] |

---

## Testing Strategy

**Testing:** [What gets tested and how — TDD, integration tests, E2E, etc.]

---

## Phases

**Phase 1 (MVP):** [X weeks] — [focus]
**Phase 2:** [focus area]
**Phase 3:** [focus area]

---

## Open Questions

<!-- TODO: [What needs deciding] | WHEN: [deadline] | BLOCKS: [what] -->

---

## Human Actions Needed

- [Service signup / decision / API key] — Before [when]

---

---

## Roadmap History

> Log of significant plan changes. Populated by `/aiagentminder:revise` when features are added, changed, dropped, or reprioritized.

<!-- | Date | Change | Reason |
|------|--------|--------|
| [DATE] | [Added / Changed / Dropped / Moved]: [feature] | [reason] | -->

---

*Unscheduled work items are captured in BACKLOG.md. Run `/aiagentminder:backlog` to add, review, or promote items.*

*Generated by /aiagentminder:brief | Last Updated: [DATE]*
